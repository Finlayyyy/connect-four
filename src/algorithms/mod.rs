...
pub use solver_manager::{SolverManager, LaissezFaire};
pub use position::Position;
pub use position::WithInfo;
...
mod position {
    ...
    pub trait Position: Board {
        ...
        fn completed(&self) -> bool { self.move_count() == MAX_MOVES }

        /// Score if the next move is a win for the current
        /// player
        fn will_win_score(&self) -> isize { ((MAX_MOVES + 1 - self.move_count()) / 2) as isize }

        /// Score if the opponents next move will let 
        /// them win. 
        fn will_lose_score(&self) -> isize { -((MAX_MOVES - self.move_count()) as isize / 2) }

        /// Score if the position was just won on the
        /// previous move
        fn just_won_score(&self) -> isize { ((MAX_MOVES + 2 - self.move_count()) / 2) as isize }

        fn curr(&self) -> Token;

        fn opp(&self) -> Token { !self.curr() }
    ...
    impl<B: Board> Position for WithInfo<B> {
        fn move_count(&self) -> usize { self.move_count }
        fn curr(&self) -> Token { self.curr }
    }
}
